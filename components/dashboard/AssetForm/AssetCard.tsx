...
function AssetCard({ asset, onDelete, view }: AssetCardProps) {
      ...
      <div className="flex items-center gap-1 p-3 rounded border border-border bg-card hover:bg-muted/40 transition-colors group">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            ...
            <Badge
              variant="outline"
              className={`text-[10px] shrink-0 ${TYPE_STYLES[asset?.type]}`}
            >
  ...
  return (
    <div className="relative group rounded overflow-hidden border border-border bg-card">
      ...
      {/* Overlay */}
      <div className="absolute inset-0 bg-linear-to-t from-black/70 via-black/20 to-transparent opacity-0 group-hover:opacity-100 transition-all duration-300 flex flex-col justify-end p-3">
      ...
      <div className="absolute top-2 left-2">
        <Badge
          variant="outline"
          className={`text-[10px] backdrop-blur-sm ${TYPE_STYLES[asset?.type]}`}
        >
          {asset?.type.toUpperCase()}
        </Badge>
      </div>
