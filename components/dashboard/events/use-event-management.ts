...
import {
  useCreateCoffeConnectMutation,
  useDeleteCoffeConnectMutation,
  useGetCoffeConnectQuery,
  useUpdateCoffeConnectMutation,
} from "@/lib/redux/services/coffeConnectApis";
import {
  CreateEventPayload,
  UpdateEventPayload,
} from "@/lib/redux/services/eventApiHelpers";
import {
  useCreateLunchAndLearnMutation,
  useDeleteLunchAndLearnMutation,
  useGetLunchAndLearnQuery,
  useUpdateLunchAndLearnMutation,
} from "@/lib/redux/services/lunchAndLearnApis";
import {
  useCreateSocialEventMutation,
  useDeleteSocialEventMutation,
  useGetSocialEventQuery,
  useUpdateSocialEventMutation,
} from "@/lib/redux/services/socialEventApis";
...
export function useEventManagement(type: EventType) {
  const coffeeQuery = useGetCoffeConnectQuery(undefined, {
    skip: type !== "coffee",
  });
  const socialQuery = useGetSocialEventQuery(undefined, {
    skip: type !== "social",
  });
  const lunchQuery = useGetLunchAndLearnQuery(undefined, {
    skip: type !== "lunch",
  });
  const query =
    type === "coffee"
  ...
  const rows = useMemo(
    () => (query.data?.data ?? []).map(normalizeEvent),
    [query.data],
  );
  const isSaving =
    createCoffeeState.isLoading ||
    createSocialState.isLoading ||
    createLunchState.isLoading ||
    updateCoffeeState.isLoading ||
    updateSocialState.isLoading ||
    updateLunchState.isLoading;
  const isDeleting =
    deleteCoffeeState.isLoading ||
    deleteSocialState.isLoading ||
    deleteLunchState.isLoading;
  ...
  async function save(data: EventFormData, event: EventRow | null) {
    try {
      ...
      if (event) {
        ...
        response =
          data.event_type === "coffee_connect"
            ? ((await createCoffee(createPayload).unwrap()) as ApiResponse)
            : data.event_type === "social_event"
              ? ((await createSocial(createPayload).unwrap()) as ApiResponse)
      ...
      toast.error(getErrorMessage(error, "Unable to save event"));
      return false;
    }
  }
  ...
  async function remove(event: EventRow) {
    try {
      const response =
        type === "coffee"
          ? ((await deleteCoffee(event.id).unwrap()) as ApiResponse)
          : type === "social"
            ? ((await deleteSocial(event.id).unwrap()) as ApiResponse)
            : ((await deleteLunch(event.id).unwrap()) as ApiResponse);
      toast.success(response.message || "Event deleted successfully");
      return true;
    } catch (error: unknown) {
      toast.error(getErrorMessage(error, "Unable to delete event"));
      return false;
    }
  }
