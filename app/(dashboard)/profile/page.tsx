...
export default function ProfilePage() {
  ...
  if (isLoading) return <><PageHeader title="Profile" description="Manage your account settings and preferences." /><div className="grid gap-6 lg:grid-cols-3"><div className="h-96 animate-pulse rounded bg-muted" /><div className="h-96 animate-pulse rounded bg-muted lg:col-span-2" /></div></>
    ...
    <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
      ...
      <div className="space-y-6 lg:col-span-2">
        <Card><CardHeader><CardTitle className="text-base">Personal Information</CardTitle></CardHeader><CardContent className="space-y-4">
          <div className="grid gap-1 sm:grid-cols-2"><div className="space-y-2"><Label htmlFor="fullName">Full Name</Label><Input id="fullName" value={profile.fullName} readOnly /></div><div className="space-y-2"><Label htmlFor="email">Email Address</Label><Input id="email" type="email" value={profile.email} readOnly /></div></div>
          <div className="grid gap-1 sm:grid-cols-2"><div className="space-y-2"><Label htmlFor="phone">Phone Number</Label><Input id="phone" value={profile.phone || "Not provided"} readOnly /></div><div className="space-y-2"><Label htmlFor="license">License Number</Label><Input id="license" value={profile.licenseNo || "Not provided"} readOnly /></div></div>
          <div className="grid gap-1 sm:grid-cols-2"><div className="space-y-2"><Label htmlFor="city">City</Label><Input id="city" value={profile.city || "Not provided"} readOnly /></div><div className="space-y-2"><Label htmlFor="country">Country</Label><Input id="country" value={profile.country || "Not provided"} readOnly /></div></div>
          <div className="space-y-2"><Label>Bio</Label><div className="min-h-20 rounded border bg-muted/20 p-3 text-sm text-muted-foreground">{profile.bio || "No bio has been added."}</div></div>
        </CardContent></Card>

        <Card><CardHeader><CardTitle className="text-base">Account & Security</CardTitle></CardHeader><CardContent className="space-y-3">
          <div className="grid gap-3 sm:grid-cols-3"><div className="rounded border p-4"><UserRound className="mb-2 h-4 w-4 text-muted-foreground" /><p className="text-xs text-muted-foreground">Account role</p><p className="mt-1 font-medium capitalize">{profile.role}</p></div><div className="rounded border p-4"><ShieldCheck className="mb-2 h-4 w-4 text-muted-foreground" /><p className="text-xs text-muted-foreground">Account status</p><p className="mt-1 font-medium">{profile.isBlocked ? "Blocked" : "In good standing"}</p></div><div className="rounded border p-4"><MapPin className="mb-2 h-4 w-4 text-muted-foreground" /><p className="text-xs text-muted-foreground">Search radius</p><p className="mt-1 font-medium">{profile.location.radiusInKm != null ? `${profile.location.radiusInKm} km` : "Not set"}</p></div></div>
          <div className="flex items-center justify-between rounded bg-muted/50 p-4"><div className="flex items-center gap-3"><span className="rounded bg-background p-2"><Key className="h-4 w-4" /></span><div><p className="text-sm font-medium">Password</p><p className="text-xs text-muted-foreground">Keep your account password secure.</p></div></div><Button variant="outline" size="sm" onClick={() => setPasswordOpen(true)}>Change</Button></div>
        </CardContent></Card>
      </div>
    </div>
