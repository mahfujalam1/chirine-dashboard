...
} from "@/app/redux-query/services/profileApis";
import { PageHeader } from "@/components/dashboard/page-header";
import {
  ChangePasswordDialog,
  EditProfileDialog,
  ProfileFormValues,
} from "@/components/dashboard/profile/profile-dialogs";
...
export default function ProfilePage() {
  ...
  async function handleProfileUpdate(values: ProfileFormValues) {
    ...
    try {
      const response = (await updateProfile(formData).unwrap()) as {
        message?: string;
      };
      ...
      toast.error(
        error?.data?.message || error?.message || "Unable to update profile",
      );
    }
  }
  ...
  return (
    <>
            ...
            <CardContent className="space-y-4">
              ...
              {/* <div className="grid gap-1 sm:grid-cols-2">
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    id="license"
                    value={profile.licenseNo || "Not provided"}
                    readOnly
                  />
                </div>
              </div>
              ...
              <div className="space-y-2">
                <Label>Bio</Label>
                <div className="min-h-20 rounded border bg-muted/20 p-3 text-sm text-muted-foreground">
            ...
            <CardContent className="space-y-3">
              <div className="grid gap-3 sm:grid-cols-2">
                <div className="rounded border p-4">
                  <UserRound className="mb-2 h-4 w-4 text-muted-foreground" />
                ...
                <div className="rounded border p-4">
                  ...
                  <p className="mt-1 font-medium">
                    {profile.isBlocked ? "Blocked" : "In good standing"}
                  </p>
                </div>
              </div>
              <div className="flex items-center justify-between rounded bg-muted/50 p-4">
                <div className="flex items-center gap-3">
                  <span className="rounded bg-background p-2">
      ...
      <EditProfileDialog
        key={`${profile.updatedAt}-${editOpen}`}
        open={editOpen}
        profile={profile}
        saving={isUpdating}
