...
const SEED_CHATS: ChatRoom[] = [
  {
    ...
    members: [
      { id: "u1", name: "Alice Martin", email: "alice@example.com", status: "active", joinedAt: "2025-09-01" },
      { id: "u2", name: "Bob Chen", email: "bob@example.com", status: "active", joinedAt: "2025-09-01" },
    ],
  },
  {
    id: "c2",
    ...
    members: [
      { id: "u3", name: "Sara Johnson", email: "sara@example.com", status: "blocked", joinedAt: "2025-09-10" },
      { id: "u4", name: "James Wilson", email: "james@example.com", status: "active", joinedAt: "2025-09-10" },
    ],
  },
  {
    id: "c3",
    ...
    members: [
      { id: "u5", name: "Noah Kim", email: "noah@example.com", status: "active", joinedAt: "2025-09-15" },
      { id: "u6", name: "Emma Davis", email: "emma@example.com", status: "active", joinedAt: "2025-09-15" },
    ],
  },
  {
    id: "c4",
    ...
    members: [
      { id: "u7", name: "Liam Brooks", email: "liam@example.com", status: "active", joinedAt: "2025-09-20" },
      { id: "u8", name: "Olivia Wright", email: "olivia@example.com", status: "active", joinedAt: "2025-09-20" },
    ],
  },
]
...
const SEED_GROUPS: GroupRoom[] = [
  {
    ...
    members: [
      { id: "u1", name: "Alice Martin", email: "alice@example.com", status: "active", isAdmin: true, joinedAt: "2025-08-01" },
      { id: "u9", name: "Mark Ellison", email: "mark@example.com", status: "active", joinedAt: "2025-08-05" },
      { id: "u10", name: "Priya Patel", email: "priya@example.com", status: "active", joinedAt: "2025-08-10" },
      { id: "u3", name: "Sara Johnson", email: "sara@example.com", status: "blocked", joinedAt: "2025-08-12" },
      { id: "u11", name: "Tom Baker", email: "tom@example.com", status: "active", joinedAt: "2025-08-15" },
      { id: "u12", name: "Lisa Nguyen", email: "lisa@example.com", status: "active", joinedAt: "2025-08-18" },
      { id: "u13", name: "Carlos Ruiz", email: "carlos@example.com", status: "active", joinedAt: "2025-08-20" },
      { id: "u14", name: "Mia Thompson", email: "mia@example.com", status: "active", joinedAt: "2025-08-22" },
    ],
  },
  {
    id: "g2",
    ...
    members: [
      { id: "u2", name: "Bob Chen", email: "bob@example.com", status: "active", isAdmin: true, joinedAt: "2025-08-01" },
      { id: "u5", name: "Noah Kim", email: "noah@example.com", status: "active", joinedAt: "2025-08-08" },
      { id: "u6", name: "Emma Davis", email: "emma@example.com", status: "active", joinedAt: "2025-08-10" },
      { id: "u15", name: "Sofia Torres", email: "sofia@example.com", status: "active", joinedAt: "2025-08-14" },
      { id: "u16", name: "Ethan Clark", email: "ethan@example.com", status: "blocked", joinedAt: "2025-08-18" },
    ],
  },
  {
    id: "g3",
    ...
    members: [
      { id: "u4", name: "James Wilson", email: "james@example.com", status: "active", isAdmin: true, joinedAt: "2025-07-01" },
      { id: "u7", name: "Liam Brooks", email: "liam@example.com", status: "active", joinedAt: "2025-07-05" },
      { id: "u8", name: "Olivia Wright", email: "olivia@example.com", status: "active", joinedAt: "2025-07-08" },
      { id: "u11", name: "Tom Baker", email: "tom@example.com", status: "active", joinedAt: "2025-07-10" },
      { id: "u17", name: "Hannah Scott", email: "hannah@example.com", status: "active", joinedAt: "2025-07-12" },
      { id: "u18", name: "Ryan Foster", email: "ryan@example.com", status: "blocked", joinedAt: "2025-07-15" },
    ],
  },
]
...
function MemberAvatar({ member, size = "sm" }: { member: RoomMember; size?: "sm" | "md" }) {
  ...
  return (
    <Avatar className={`${sz} shrink-0`}>
      ...
      <AvatarFallback className={`${txt} font-medium`}>{initials(member.name)}</AvatarFallback>
    </Avatar>
  )
}
...
function RoomDetailDialog({ room, onClose, onToggleMemberBlock, onToggleRoomBlock }: RoomDetailProps) {
  const [confirmBlock, setConfirmBlock] = useState<{ memberId: string; action: "block" | "unblock" } | null>(null)

  if (!room) return null

  const blockedCount = room.members.filter((m) => m.status === "blocked").length
    ...
    <>
        ...
        <DialogContent className="max-w-md max-h-[90vh] flex flex-col">
          <DialogHeader>
            <div className="flex items-start gap-3 pr-6">
              ...
              <div className="min-w-0">
                <DialogTitle className="text-base leading-tight truncate">{room.roomName}</DialogTitle>
                <div className="flex items-center gap-2 mt-1.5 flex-wrap">
                  <Badge variant="outline" className="text-xs capitalize">{room.type}</Badge>
                  <Badge
                    variant="outline"
                    className={room.isBlocked
                      ? "text-xs bg-red-50 text-red-700 border-red-200 dark:bg-red-950/40 dark:text-red-400 dark:border-red-800"
                      : "text-xs bg-emerald-50 text-emerald-700 border-emerald-200 dark:bg-emerald-950/40 dark:text-emerald-400 dark:border-emerald-800"
                    }
                  >
                  ...
                  {blockedCount > 0 && (
                    <Badge variant="outline" className="text-xs bg-amber-50 text-amber-700 border-amber-200 dark:bg-amber-950/40 dark:text-amber-400 dark:border-amber-800">
                ...
                {"topic" in room && room.topic && (
                  <p className="text-xs text-muted-foreground mt-1.5 flex items-center gap-1">
                    <Hash className="w-3 h-3" />{room.topic}
                  </p>
                )}
              </div>
            </div>
          ...
          <div className="pt-3 border-t border-border mt-2">
            <Button
              variant="outline"
              size="sm"
              className={`w-full ${room.isBlocked
            ...
            >
              {room.isBlocked ? (
                <><ShieldCheck className="w-4 h-4 mr-2" />Unblock Entire Room</>
              ) : (
                <><ShieldOff className="w-4 h-4 mr-2" />Block Entire Room</>
              )}
            </Button>
          </div>
        </DialogContent>
      ...
      <AlertDialog open={!!confirmBlock} onOpenChange={(o) => !o && setConfirmBlock(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
              {confirmBlock?.action === "block" ? "Block this member?" : "Unblock this member?"}
            </AlertDialogTitle>
            <AlertDialogDescription>
          ...
          <AlertDialogFooter>
            <AlertDialogCancel onClick={() => setConfirmBlock(null)}>Cancel</AlertDialogCancel>
            <AlertDialogAction
              className={confirmBlock?.action === "block"
            ...
            >
              {confirmBlock?.action === "block" ? "Block Member" : "Unblock Member"}
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
...
export default function ChatManagementPage() {
  ...
  function toggleRoomBlock(roomId: string) {
    const update = (rooms: Room[]) =>
      rooms.map((r) => (r.id === roomId ? { ...r, isBlocked: !r.isBlocked } : r))
  ...
  const chatColumns: ColumnDef<ChatRoom>[] = [
    {
      ...
      renderItem: (record) => (
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-sm font-medium text-foreground">{record.roomName}</p>
            <p className="text-xs text-muted-foreground">{record.memberCount} members</p>
          </div>
        </div>
      ),
    },
    ...
    },
    {
      ...
      key: "lastActivity",
      renderItem: (record) => (
        <span className="text-sm text-muted-foreground">{record.lastActivity}</span>
      ),
    },
    {
      title: "Status",
      ...
      renderItem: (record) => (
        <div className="flex items-center justify-end gap-1">
          <Button
            ...
            className="h-8 w-8"
            title="View room"
            onClick={(e) => { e.stopPropagation(); setDetailRoom(record) }}
          >
          ...
          <Button
            variant="ghost"
            size="icon"
            className={`h-8 w-8 ${record.isBlocked
              ? "text-emerald-600 hover:text-emerald-700 hover:bg-emerald-50 dark:hover:bg-emerald-950/40"
              : "text-destructive hover:text-destructive hover:bg-destructive/10"
              }`}
            title={record.isBlocked ? "Unblock room" : "Block room"}
            onClick={(e) => { e.stopPropagation(); toggleRoomBlock(record.id) }}
          >
            {record.isBlocked ? <ShieldCheck className="w-4 h-4" /> : <ShieldOff className="w-4 h-4" />}
          </Button>
        </div>
      ),
    },
      ...
      renderItem: (record) => (
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-sm font-medium text-foreground">{record.roomName}</p>
            {record.topic && (
              <p className="text-xs text-muted-foreground truncate max-w-50">{record.topic}</p>
            )}
          </div>
        </div>
      ),
    },
    {
      ...
      renderItem: (record) => (
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs text-muted-foreground">{record.memberCount}</span>
        </div>
      ),
    },
    {
      title: "Blocked",
      renderItem: (record) => {
        const blocked = record.members.filter((m) => m.status === "blocked").length
        return blocked > 0 ? (
          <Badge variant="outline" className="text-xs bg-amber-50 text-amber-700 border-amber-200 dark:bg-amber-950/40 dark:text-amber-400 dark:border-amber-800">
            {blocked} blocked
          </Badge>
    ...
    <Suspense fallback={<Loading />}>
      <>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-4 gap-4 mb-6">
          ...
          <Card
            className={`bg-card border border-border cursor-pointer hover:border-foreground/30 transition-all ${tab === "groups" ? "ring-2 ring-foreground/20" : ""}`}
          ...
          >
            <CardContent className="p-5">
              ...
              <p className="text-xs text-muted-foreground mt-1">multi-member groups</p>
            </CardContent>
          </Card>
          <Card className="bg-card border border-border">
            <CardContent className="p-5">
              ...
              <p className="text-xs text-muted-foreground mt-1">across all rooms</p>
            </CardContent>
          </Card>
          <ActiveTherapist />
        </div>
          ...
          <CardHeader className="pb-2">
            <div className="flex items-center justify-between gap-4 flex-wrap">
              ...
              <div className="relative w-64">
                ...
                <Input
                  placeholder={tab === "chat" ? "Search rooms or members…" : "Search groups or members…"}
                  value={searchQuery}
            ...
            <div className="flex items-center justify-between gap-3 mb-4 flex-wrap">
              <div className="flex items-center gap-2">
                {(["chat", "groups"] as const).map((t) => (
                  ...
                  >
                    {t === "chat" ? (
                      <><MessageCircleMore className="w-3.5 h-3.5 mr-1.5" />Chats</>
                    ) : (
                      <><Boxes className="w-3.5 h-3.5 mr-1.5" />Groups</>
                    )}
                  </Button>
                ))}
              </div>
