  ...
  const columns: ColumnDef<RequestEvent>[] = [
    {
      ...
      renderItem: (event) => <div className="flex min-w-64 items-center gap-3">
        <Image src={event.image || "/placeholder.svg"} alt={event.title} width={64} height={48} className="h-12 w-16 rounded bg-muted object-cover" />
    ...
    <Dialog open={!!selectedEvent} onOpenChange={(open) => !open && setSelectedEvent(null)}>
      <DialogContent className="max-h-[90vh] max-w-2xl overflow-y-auto">
        {selectedEvent && <>
          ...
          {selectedEvent.image && <Image src={selectedEvent.image} alt={selectedEvent.title} width={600} height={300} className="max-h-72 w-full rounded bg-muted object-cover" />}
