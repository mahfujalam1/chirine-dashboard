...
} from "@/app/redux-query/services/reportApis";
...
export default function ReportsPage() {
  ...
  return (
    <>
      ...
      />
      <div className="mb-1 grid grid-cols-2 gap-1 lg:grid-cols-4">
        {metrics.map((metric) => (
          <button
            key={metric.label}
            className={`rounded text-left transition-all ${status === metric.filter ? "bg-gray-300" : ""}`}
            onClick={() => {
