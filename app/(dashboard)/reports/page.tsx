...
export default function ReportsPage() {
  ...
  return <>
    ...
    <div className="mb-1 grid grid-cols-2 gap-1 lg:grid-cols-4">{metrics.map((metric) => <button key={metric.label} className={`rounded text-left ring-offset-background transition-all ${status === metric.filter ? "ring-2 ring-foreground/20" : ""}`} onClick={() => { setStatus(metric.filter); setPage(1) }}><MetricCard title={metric.label} value={String(metric.value)} isPositiveOutcome icon={metric.icon} /></button>)}</div>
