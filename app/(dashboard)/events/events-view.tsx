  ...
  return (
    <>
      <PageHeader title="Events" description="Create and manage published events by category.">
        <Button className="bg-[#00ACA7] text-white" onClick={openCreateForm}>
      ...
      <Card>
        <CardContent className="p-5">
          <div className="mb-5 flex gap-2 overflow-x-auto">
            {EVENT_TABS.map((tab) => (
              <Button
                ...
                className={type === tab.value ? "bg-[#00ACA7] text-white" : ""}
          ...
          {query.isError ? (
            <div className="flex flex-col items-center rounded border border-destructive/30 bg-destructive/5 p-8 text-center">
              <p className="text-sm font-medium text-destructive">Unable to load {currentTab.label}</p>
            ...
            <EventsTable
              ...
              emptyText={`No ${currentTab.label} found.`}
              onEdit={openEditForm}
              onDelete={setDeleteTarget}
              onParticipants={setParticipantsEvent}
            />
          )}
        </CardContent>
      </Card>
