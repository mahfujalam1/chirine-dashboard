...
} from "@/lib/redux/services/professionApis";
...
export default function ProfessionsClientView() {
  ...
  async function handleSave(input: ProfessionInput) {
    try {
      ...
      toast.success(
        response.message ||
          `Profession ${editTarget ? "updated" : "created"} successfully`,
      );
      setFormOpen(false);
      setEditTarget(null);
    } catch (error: unknown) {
          ...
          {professionsQuery.isError ? (
            <div className="flex flex-col items-center rounded-lg border border-destructive/30 bg-destructive/5 p-10">
              <p className="text-sm text-destructive">
                Unable to load professions.
              </p>
              <Button
                variant="outline"
                size="sm"
                className="mt-3"
                onClick={() => professionsQuery.refetch()}
              >
            ...
            <ProfessionsTable
              ...
              loading={
                professionsQuery.isLoading || professionsQuery.isFetching
              }
              meta={{
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              “{deleteTarget?.name}” will be permanently deleted. Governing
